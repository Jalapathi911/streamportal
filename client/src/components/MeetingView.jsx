...
export default function MeetingView({ roomId, onLeave }) {
  ...
  return (
    ...
    >
      ...
      {/* Back button — top-left corner, fades with controls in fullscreen */}
      {onLeave && (
        <button
          onClick={(e) => { e.stopPropagation(); onLeave(); }}
          style={{ transition: 'opacity 0.3s ease', opacity: controlsVisible ? 1 : 0, pointerEvents: controlsVisible ? 'auto' : 'none' }}
          className="absolute top-4 left-4 z-20 w-9 h-9 rounded-full flex items-center justify-center bg-black/60 hover:bg-black/80 text-white shadow-lg"
        >
          <BackIcon />
        </button>
      )}
